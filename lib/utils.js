...
exports.parseHost = function (req, hostHeaderName) {
    ...
    // Handle IPv6 format and standard hostname:port format
    var name, port;

    // Special handling for IPv6 with port
    if (hostHeader[0] === '[') {
        var rightBracketPosition = hostHeader.indexOf(']');
        if (rightBracketPosition === -1) {
            return null;
        }

        name = hostHeader.slice(0, rightBracketPosition + 1);

        if (hostHeader[rightBracketPosition + 1] === ':') {
            port = hostHeader.slice(rightBracketPosition + 2);
            // Make sure port is only digits
            if (!port || !/^\d+$/.test(port)) {
                return null;
            }
        }
        ...
        var colonPosition = hostHeader.indexOf(':');
        if (colonPosition !== -1) {
            name = hostHeader.slice(0, colonPosition);
            port = hostHeader.slice(colonPosition + 1);
            // Make sure port is only digits and hostname is not empty
            if (!port || !name || !/^\d+$/.test(port)) {
                return null;
            }
        }
        else {
